 ...
 */
async function hasNativePhp(): Promise<boolean> {
  try {
 ...
 */
function parsePhpLintOutput(filePath: string, output: string): PhpSyntaxError[] {
  const errors: PhpSyntaxError[] = []
 ...
 */
async function collectPhpFiles(targetPath: string): Promise<string[]> {
 ...
 */
export async function checkPhpSyntax(targetPath: string): Promise<CheckPhpSyntaxResult> {
