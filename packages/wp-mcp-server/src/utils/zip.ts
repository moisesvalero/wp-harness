 ...
 */
export async function createZipArchive(sourceDir: string, targetZipPath: string): Promise<{ sizeBytes: number }> {
  ...
  try {
    ...
    throw new Error(`Failed to create zip archive with zip CLI: ${(nativeError as Error).message}`)
  }
}
