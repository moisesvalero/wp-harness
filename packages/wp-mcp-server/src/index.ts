...
export function createWordPressMcpServer(): McpServer {
  ...
  server.tool(
    'wp_scaffold_theme',
    'Scaffolds a modern WordPress Block Theme with theme.json v3, style.css, template parts, and functions.php',
    {
  ...
  server.tool(
    'check_php_syntax',
    'Runs php -l syntax check across PHP files and parses any syntax errors into structured actionable JSON',
    {
  ...
  server.tool(
    'verify_in_playground',
    'Mounts the theme into an ephemeral WP Playground CLI instance, executes a headless healthcheck, and asserts HTTP 200 with zero PHP fatal errors',
    {
  ...
  server.tool(
    'finish_task',
    'Terminal stopping tool that validates all project files, verifies against WordPress Coding Standards and PHP syntax, and exports to dist/site.zip',
    {
